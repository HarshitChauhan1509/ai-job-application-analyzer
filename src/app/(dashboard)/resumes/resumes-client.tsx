...
export default function ResumesClient({ initialResumes, userId }: { initialResumes: any[]; userId: string }) {
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {initialResumes.length === 0 ? (
          ...
          initialResumes.map((resume) => {
            const score = resume.analysis?.overallScore || 0;
                ...
                <CardContent className="pb-4 flex-1">
                  <div className="rounded-md bg-muted/50 p-4 flex flex-col items-center justify-center text-center h-full border border-border/50">
                    ...
                    <div className="w-full max-w-[120px] h-1.5 bg-muted-foreground/20 rounded-full mt-3 overflow-hidden">
                      <div 
                        className={`h-full rounded-full ${score >= 90 ? 'bg-green-500' : score >= 70 ? 'bg-primary' : 'bg-amber-500'}`} 
                        style={{ width: `${score}%` }}
                      />
                    </div>
                  </div>
